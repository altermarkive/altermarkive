# Exploring Azure Services

* [Uploading to Azure Blob Storage](blob-upload)
* [Password-protected, App Service (Python, Flask & Docker) with Swagger API and static frontend served by Nginx](app-service)
* [Container Registry creation script](container-registry-creation)
* [Multicontainer App Service with Volume](multicontainer-app-service-with-volume)
* [Kubernetes: AKS with authenticated App Service ingress](aks-with-app-service-ingress)
* [Azure VPN Gateway & on-premise facility integration](on-premise-integration/README.md)
* [Azure IoT Edge: Containerized](iot-edge-containerized)
* [Azure IoT Edge: Module & service boilerplate example](iot-edge-module)
* [Azure IoT Edge: Provisioning with Ansible](iot-edge-with-ansible)
* [Azure IoT Edge: Virtual Kubelet & AKS](iot-edge-with-virtual-kubelet-and-aks)
